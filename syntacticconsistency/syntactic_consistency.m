function syntactic_consistency
clc
clear all
...
local = 'dataBaseSyntacticConsistency\';
typeofscore='scanned';
withbarlines='yes';
...
% Read data from the references file
...
%img_all = double(imread('img004.png'));
...
for idxnames=1:length(names)
    ...
    for numberofstaffs=1:size(allstaffs,2)
        ...
        switch withbarlines
            case 'yes'
                [~,~,DATAPROB,data_time,typeTime] = read_timesignatures(TimeSignatures,DATAPROB,originalscore);
                numerator = numerator_(numberofstaffs);
                denominator = denominator_(numberofstaffs);
            case 'no'
                [numerator,denominator,DATAPROB,data_time,typeTime] = read_timesignatures(TimeSignatures,DATAPROB,originalscore);
        end
        ...
        switch typeofmodel
            ...
            case 'nn'
                ...
                IDX = [IDX find(predicted_label == 6)];
                ...
                IDX = [IDX find(predicted_label == 16)];
                ...
                IDX = [IDX find(predicted_label == 12)];
                ...
                IDX = [IDX find(predicted_label == 15)];
                ...
                IDX = [IDX find(predicted_label == 14)];
                ...
                IDX = [IDX find(predicted_label == 1)];
                ...
                IDX = [IDX find(predicted_label == 11)];
                ...
                IDX = [IDX find(predicted_label == 10)];
                if size(idx,2) ~= 0
                ...
                IDX = [IDX find(predicted_label == 8)];
                ...
                IDX = [IDX find(predicted_label == 9)];
                ...
                IDX = [IDX find(predicted_label == 17)];
                ...
                symbolsposition_all(IDX,:) = [];
                prob_estimates(:,IDX)=[];
                DATAPROB = setfield(DATAPROB, 'others', prob_estimates);
                
                size(data_time)
                size(data_clef)
                size(BarLines)
                
                disp('opennoteheadwithoutbeam-----------------------------')
                size(setBreve)
                size(setSemibreve)
                size(OpenNoteHeadWithoutStem)
                
                disp('minim-----------------------------')
                size(setMinim)
                size(OpenNoteheadstem)
        ...
        alpha = [0 0 2 2+2*1/2 1 1+1*1/2 1/2 1/2+1/2*1/2 1/4 1/4+1/4*1/2 1/8 1/8+1/8*1/2 1/16 1/16+1/16*1/2 1/32 1/32+1/32*1/2 1/64 1/64+1/64*1/2 ...
        ...
        for i=1:size(BarLines,1)-1
            ...
            if size(symbolsposition_all,1)~=0
                columns = symbolsposition_all(:,3);
                idx = find(columns >= begining & columns < ending);
                syb = symbolsposition_all(idx,:);
                %                 prob = DATAPROB.others;
                %                 PROB = prob(idx,:);
                MATRIXSYMBOLS = [MATRIXSYMBOLS; syb repmat(0,size(idx,1),1)];
                MATRIXPROB = [MATRIXPROB; repmat(1/36,size(syb,1),36)];
                %degree(19,2)
            end
            if size(setBreve,1)~=0
                columns = setBreve(:,3);
                idx = find(columns >= begining & columns < ending);
                PROB = PROB_6(idx,:);
                MATRIXSYMBOLS = [MATRIXSYMBOLS; setBreve(idx,:) repmat(0,size(idx,1),1)];
                ...
                if dg == 1
                    ...
                    MATRIXPROB = [MATRIXPROB; auxMatrixProb];
                end
                %degree(19,2)
            end
            if size(setSemibreve,1)~=0
                ...
                idx = find(columns >= begining & columns < ending);
                PROB = PROB_7(idx,:);
                MATRIXSYMBOLS = [MATRIXSYMBOLS; setSemibreve(idx,:) repmat(0,size(idx,1),1)];
                auxMatrixProb = [PROB(:,15) PROB(:,17) PROB(:,33) PROB(:,1) PROB(:,11:12) PROB(:,31:32) PROB(:,23:24) PROB(:,19:20) PROB(:,21:22) ...
                    PROB(:,21:22) PROB(:,21:22) PROB(:,21:22) PROB(:,29:30) PROB(:,29:30) PROB(:,29:30) PROB(:,27:28) PROB(:,29:30) PROB(:,29:30) PROB(:,29:30)...
                    PROB(:,29:30)];
                if dg == 1
            ...
            if size(setMinim,1)~=0
                ...
                idx = find(columns >= begining & columns < ending);
                PROB = PROB_8(idx,:);
                MATRIXSYMBOLS = [MATRIXSYMBOLS; setMinim(idx,:) repmat(0,size(idx,1),1)];
                auxMatrixProb = [PROB(:,15) PROB(:,17) PROB(:,33) PROB(:,1) PROB(:,11:12) PROB(:,31:32) PROB(:,23:24) PROB(:,19:20) PROB(:,21:22) ...
                    PROB(:,21:22) PROB(:,21:22) PROB(:,21:22) PROB(:,29:30) PROB(:,29:30) PROB(:,29:30) PROB(:,27:28) PROB(:,29:30) PROB(:,29:30) PROB(:,29:30)...
                    PROB(:,29:30)];
                if dg == 1
            ...
            if size(setRests,1)~=0
                ...
                idx = find(columns >= begining & columns < ending);
                PROB = PROB_5(idx,:);
                MATRIXSYMBOLS = [MATRIXSYMBOLS; setRests(idx,:) repmat(0,size(idx,1),1)];
            ...
            if size(setRests1,1)~=0
                ...
                idx = find(columns >= begining & columns < ending);
                PROB = PROB_4(idx,:);
                MATRIXSYMBOLS = [MATRIXSYMBOLS; setRests1(idx,:) repmat(0,size(idx,1),1)];
            ...
            if size(setAccents,1) ~=0
                ...
                if size(idx,1) ~= 0
                    ...
                    MATRIXSYMBOLS = [MATRIXSYMBOLS; setAccents(idx,:) repmat(0,size(idx,1),1)];
            ...
            if size(setFlags,1)~=0
                ...
                idx = find(columns >= begining & columns < ending);
                PROB = PROB_1(idx,:);
                MATRIXSYMBOLS = [MATRIXSYMBOLS; setFlags(idx,:) repmat(0,size(idx,1),1)];
            ...
            if size(setFlagsBeams,1)~=0
                ...
                idx = find(columns >= begining & columns < ending);
                PROB = PROB_2(idx,:);
                MATRIXSYMBOLS = [MATRIXSYMBOLS; setFlagsBeams(idx,:) repmat(1,size(idx,1),1)];
                auxMatrixProb = [repmat(0,size(PROB,1),12) PROB(:,5:6) PROB(:,7:8) PROB(:,1:2) PROB(:,3:4) repmat(0,size(PROB,1),16)];
                if dg == 1
                    matrixDegree = [repmat((1-degrees(7,2))/28,size(PROB,1),12) repmat(degrees(7,2),size(PROB,1),8) repmat((1-degrees(7,2))/28,size(PROB,1),16)];
                    MATRIXPROB = [MATRIXPROB; auxMatrixProb.*matrixDegree];
            ...
            if size(setQuarter,1)~=0
                ...
                idx = find(columns >= begining & columns < ending);
                PROB = PROB_3(idx,:);
                MATRIXSYMBOLS = [MATRIXSYMBOLS; setQuarter(idx,:) repmat(0,size(idx,1),1)];
                auxMatrixProb = [PROB(:,15) PROB(:,17) PROB(:,33) PROB(:,1) PROB(:,11:12) PROB(:,31:32) PROB(:,23:24) PROB(:,19:20) PROB(:,21:22) ...
                    PROB(:,21:22) PROB(:,21:22) PROB(:,21:22) PROB(:,29:30) PROB(:,29:30) PROB(:,29:30) PROB(:,27:28) PROB(:,29:30) PROB(:,29:30) PROB(:,29:30)...
                    PROB(:,29:30)];
                if dg == 1
            ...
            if size(setFlat,1)~=0
                ...
                PROB_9(:,1:2:40) = data;
                PROB = PROB_9(idx,:);
                MATRIXSYMBOLS = [MATRIXSYMBOLS; setFlat(idx,:) repmat(0,size(idx,1),1)];
                auxMatrixProb = [PROB(:,15) PROB(:,17) PROB(:,33) PROB(:,1) PROB(:,11:12) PROB(:,31:32) PROB(:,23:24) PROB(:,19:20) PROB(:,21:22) ...
                    PROB(:,21:22) PROB(:,21:22) PROB(:,21:22) PROB(:,29:30) PROB(:,29:30) PROB(:,29:30) PROB(:,27:28) PROB(:,29:30) PROB(:,29:30) PROB(:,29:30)...
                    PROB(:,29:30)];
                if dg == 1
            ...
            if size(setNatural,1)~=0
                ...
                PROB_10(:,1:2:40) = data;
                PROB = PROB_10(idx,:);
                MATRIXSYMBOLS = [MATRIXSYMBOLS; setNatural(idx,:) repmat(0,size(idx,1),1)];
                auxMatrixProb = [PROB(:,15) PROB(:,17) PROB(:,33) PROB(:,1) PROB(:,11:12) PROB(:,31:32) PROB(:,23:24) PROB(:,19:20) PROB(:,21:22) ...
                    PROB(:,21:22) PROB(:,21:22) PROB(:,21:22) PROB(:,29:30) PROB(:,29:30) PROB(:,29:30) PROB(:,27:28) PROB(:,29:30) PROB(:,29:30) PROB(:,29:30)...
                    PROB(:,29:30)];
                if dg == 1
            ...
            if size(setSharp,1)~=0
                ...
                PROB_11(:,1:2:40) = data;
                PROB = PROB_11(idx,:);
                MATRIXSYMBOLS = [MATRIXSYMBOLS; setSharp(idx,:) repmat(0,size(idx,1),1)];
                auxMatrixProb = [PROB(:,15) PROB(:,17) PROB(:,33) PROB(:,1) PROB(:,11:12) PROB(:,31:32) PROB(:,23:24) PROB(:,19:20) PROB(:,21:22) ...
                    PROB(:,21:22) PROB(:,21:22) PROB(:,21:22) PROB(:,29:30) PROB(:,29:30) PROB(:,29:30) PROB(:,27:28) PROB(:,29:30) PROB(:,29:30) PROB(:,29:30)...
                    PROB(:,29:30)];
                if dg == 1
            ...
            %             img = originalscore;
            %             for j=1:10
            %                 originalscore(MATRIXSYMBOLS(j,1):MATRIXSYMBOLS(j,2),MATRIXSYMBOLS(j,3):MATRIXSYMBOLS(j,4))=0.5;
            %             end
            %             imwrite(originalscore,'xx.png','png')
            %
            %             for j=11:size(MATRIXSYMBOLS,1)
            %                 img(MATRIXSYMBOLS(j,1):MATRIXSYMBOLS(j,2),MATRIXSYMBOLS(j,3):MATRIXSYMBOLS(j,4))=0.5;
            %             end
            %             imwrite(img,'xx1.png','png')
            ...
            [~,id] = sort(MATRIXSYMBOLS(:,3));
            MATRIXSYMBOLS = MATRIXSYMBOLS(id,:);
            MATRIXPROB(:,[2 3]) = [];
            MATRIXPROB = MATRIXPROB(id,:);
            ...
            if size(newx,1) ~= 0
                ...
                a = MATRIXSYMBOLS(:,1:2);
                newsymbols = [a + repmat(initialpositions(numberofstaffs),size(a)) MATRIXSYMBOLS(:,3:5)];
                
                %                 img_all = double(imread('img009.png'));
                %                 for j=1:size(newsymbols,1)
                %                     img_all(newsymbols(j,1):newsymbols(j,2),newsymbols(j,3):newsymbols(j,4))=0.5;
                %                 end
        ...
        a_ = BarLines(:,1:2);
        newsymbols_ = [a_ + repmat(initialpositions(numberofstaffs),size(a_)) BarLines(:,3:4) repmat(0, size(a_,1),1)];
        SYMBOLS = [SYMBOLS; newsymbols_ repmat(37, size(newsymbols_,1),1) repmat(0, size(newsymbols_,1),3)];
        
        if size(data_time,1) ~= 0
            a_ = data_time(:,1:2);
            newsymbols_ = [a_ + repmat(initialpositions(numberofstaffs),size(a_)) data_time(:,3:4) repmat(0, size(a_,1),1)];
            if typeTime == 1
                SYMBOLS = [SYMBOLS; newsymbols_ repmat(38, size(newsymbols_,1),1) repmat(0, size(newsymbols_,1),3)];
            else
                SYMBOLS = [SYMBOLS; newsymbols_ repmat(39, size(newsymbols_,1),1) repmat(0, size(newsymbols_,1),3)];
            end
        end
        
        if size(data_clef,1) ~= 0
            a_ = data_clef(:,1:2);
            newsymbols_ = [a_ + repmat(initialpositions(numberofstaffs),size(a_)) data_clef(:,3:4) repmat(0, size(a_,1),1)];
            SYMBOLS = [SYMBOLS; newsymbols_ repmat(40, size(newsymbols_,1),1) repmat(0, size(newsymbols_,1),3)];
        end
        
        if size(setKeyFlat,1) ~= 0
            a_ = setKeyFlat(:,1:2);
            newsymbols_ = [a_ + repmat(initialpositions(numberofstaffs),size(a_)) setKeyFlat(:,3:4) repmat(0, size(a_,1),1)];
            SYMBOLS = [SYMBOLS; newsymbols_ repmat(1, size(newsymbols_,1),1) repmat(0, size(newsymbols_,1),3)];
        end
        
        if size(setKeyNatural,1) ~= 0
            a_ = setKeyNatural(:,1:2);
            newsymbols_ = [a_ + repmat(initialpositions(numberofstaffs),size(a_)) setKeyNatural(:,3:4) repmat(0, size(a_,1),1)];
            SYMBOLS = [SYMBOLS; newsymbols_ repmat(2, size(newsymbols_,1),1) repmat(0, size(newsymbols_,1),3)];
        end
        
        if size(setKeySharp,1) ~= 0
            a_ = setKeySharp(:,1:2);
            newsymbols_ = [a_ + repmat(initialpositions(numberofstaffs),size(a_)) setKeySharp(:,3:4) repmat(0, size(a_,1),1)];
            SYMBOLS = [SYMBOLS; newsymbols_ repmat(3, size(newsymbols_,1),1) repmat(0, size(newsymbols_,1),3)];
        end
