function [object,flagcontrolsize]=BreadthFirstSearch_controlsize(P, w, h, imageWithoutLines,threshold,spaceHeight)
...
while z~=0
    ...
    if visitedMatrix(M(1),M(2))==1
        ...
        newSymbol=[yyy(1) yyy(end) xxx(1) xxx(end)];
        widthobject = newSymbol(4)-newSymbol(3)+1;
        
        if widthobject > 6*spaceHeight
            object=[];
            flagcontrolsize=1;
            return
        end
